...
function CartSummary(props) {
  ...
  for (const key in currentCart) {
    cartItemElements.push(<CartSummaryItem key={key} image={currentCart[key].product.image} productName={currentCart[key].product.name} price={currentCart[key].product.price * currentCart[key].quantity} description={currentCart[key].product.shortDescription} quantity={currentCart[key].quantity} productId={currentCart[key].product.productId} removeItem={props.removeItem} addItem={props.addItem} product={currentCart[key].product} showWarningModal={props.showWarningModal} />);
  }
  let totalPrice = 0;
  ...
  return (
    <div className="container">
      ...
      <div className="row">
        ...
        <div><button className="btn btn-success" onClick={() => props.setView('checkout', {})}>Checkout</button></div>
      </div>
    </div>);
}
